...
class TestVideoMisuseAnalyzer(unittest.TestCase):
    ...
    def test_analyze_all_csv_user_id_column(self):
        ...
        for csv_file in csv_files:
            if "user" in csv_file.name.lower() and "profile" in csv_file.name.lower():
        ...
        with open(user_profiles_csv, 'r', encoding='utf-8') as f:
            ...
            self.assertLess(user_id_index, 5, "user_id column should be in the first few columns")
            ...
            for row in rows:
                ...
                self.assertNotEqual(user_id_value, "", f"Empty user_id found in row: {row}")
            ...
            if isinstance(json_data, dict):
                if "user_profiles" in json_data:
                    ...
                    for user_id, profile in json_data["user_profiles"].items():
                        self.assertIn("user_id", profile, "user_id field missing from user profile")
                        self.assertNotEqual(profile["user_id"], "", "user_id should not be empty")
    ...
    def test_video_misuse_detection_processing(self):
        ...
        self.assertGreater(results.get("video_misuse_cases", 0), 0, "No video misuse cases detected")
        ...
        if violation_files:  # Only check if violation files exist
            with open(violation_files[0], 'r', encoding='utf-8') as f:
                ...
                self.assertGreater(len(violations), 0, "No violations found in violation details")
                
                # Check that user_id is present in violation records
                if violations:
                    self.assertIn("user_id", violations[0], "user_id missing from violation record")
    ...
    def test_csv_fieldnames_order(self):
        ...
        if csv_files:
            with open(csv_files[0], 'r', encoding='utf-8') as f:
                ...
                screen_name_pos = headers.index("screen_name") if "screen_name" in headers else -1
                
                # user_id should come after screen_name if both exist
                if screen_name_pos >= 0:
                    self.assertGreater(user_id_pos, screen_name_pos, 
